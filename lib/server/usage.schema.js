import { z } from "zod";
//#region src/server/usage.schema.ts
/**
...
const usageDimensionSchema = z.enum([
	"day",
	"model",
	"session",
	"workspace"
...
const usageDateSchema = z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "expected YYYY-MM-DD");
/** usage.query row ordering. */
const usageSortSchema = z.enum(["tokens-desc", "tokens-asc"]);
/** usage.query request payload. */
const usageQueryRequestSchema = z.object({
...
const usageTokenTotalsSchema = z.object({
...
const usageRowSchema = usageTokenTotalsSchema.extend({
...
const usageQueryValueSchema = z.object({
